...
close_date[["io/andy_output_1694.json"]] <- "2018-05-31"
close_date[["io/andy_output_2099.json"]] <- "2018-06-28"
...
make_table <- function(files) {
  ...
  for (fh in files) {
    ...
    this_ifp <- list()
    if (!is.null(response$estimated) | !is.null(response$message)) {
      this_ifp <- data.frame(
        IFP = request_no, 
        Model = "ARIMA", 
        N = NA,
      ...
      this_ifp <- data.frame(
        ...
        data_agg = ifelse(
          response$parsed_request$was_data_aggregated,
          "yes",
          "no"),
